...
public class FoodMapController {
	...
	public FoodMapRes setStore(@RequestBody FoodMapReq req) {
		...
		//判斷建立城市與店家資料時是否輸入完整																	
		if(!StringUtils.hasText(req.getCity()) || !StringUtils.hasText(req.getStoreName())) {															
			return new FoodMapRes(FoodMapRtnCode.CITY_OR_STORE_CANNOT_BE_EMPTY.getMessage()); 			
		}
		
		// 若資料無誤則建立店家資料
		Store store = foodMapService.setStore(req.getCity(), req.getStoreName());
	...
	public FoodMapRes changeInfoForStore(@RequestBody FoodMapReq req) {
		...
		// 若城市與新店名沒有輸入值，判斷為取消修改
		if(!StringUtils.hasText(req.getCity()) && !StringUtils.hasText(req.getNewStoreName())) {	
			return new FoodMapRes(FoodMapRtnCode.CANCEL_CHANGES.getMessage());
		}
		
		// 若判斷資料無誤則存入新修改的店家資料
		Store store = foodMapService.changeInfoForStore(req.getCity(), req.getStoreName(), req.getNewStoreName());
		if(store == null) {
	...
	public FoodMapRes setMeals(@RequestBody FoodMapReq req) {											
		...
		// 判斷資料無誤直接存入 food
		Food food = foodMapService.setMeals(req.getStoreName(), req.getMealsName(), req.getMealsPrice(), req.getMealsAssess());
	...
	private FoodMapRes checkMap(FoodMapReq req) { 	
		...
		// 判斷使用者輸入的餐點名稱是否為空的
		} else if (!StringUtils.hasText(req.getMealsName())) { 			
	...
	public FoodMapRes changeInfoforMeals(@RequestBody FoodMapReq req) {
		...
		if(!StringUtils.hasText(req.getMealsName()) || !StringUtils.hasText(req.getStoreName())) {		
		...
		Food food = foodMapService.changeInfoForMeals(req.getStoreName(), req.getMealsName(), req.getNewMealsName(), req.getMealsPrice(), req.getMealsAssess());
		if(food == null) {
			return new FoodMapRes(FoodMapRtnCode.STORE_OR_MEALS_ALREADY_EXISTED.getMessage());
		}
	...
	public FoodMapResponseList findStoreByCity(@RequestBody FoodMapReq req) {
		...
		if(!StringUtils.hasText(req.getCity())) {															
			return new FoodMapResponseList(FoodMapRtnCode.CITY_OR_STORE_CANNOT_BE_EMPTY.getMessage());
		}
	...
	public FoodMapRes findByStoreAssessGreaterThanEqual(@RequestBody FoodMapReq req) {
		...
		if(checkAssess(req.getStoreAssess())) {														//判斷要查詢的店家評價是否小於0或大於5
	...
	public FoodMapRes findStoreAssessAndMealsAssess(@RequestBody FoodMapReq req) {
		...
		if(checkAssess(((double)req.getMealsAssess()))) {									//判斷要查詢的餐點評價是否小於0或大於5
	...
	//定義私有化
	private boolean checkAssess(double assess) {
		return assess < 1 || assess > 5;
	}
}
